<template>
  <div id="strategyUddate" ref='132'>
    <div class="strategyTitle"><img src="../../assets/logo/title.png" />策略修改</div>
    <div class="content">
        ...
        <el-form :gutter="24" label-width="120px" class="form" >
          <el-col :span="12">
            <el-form-item label="策略名称：">
              <el-input id="inputGet" @blur="handleBlue" v-model="$store.state.strategy.strategyName" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="选择群组：">
              <div class="flex alignItem">
                <el-select v-model="$store.state.strategy.groupId" filterable disabled>
                  <el-option clearable v-for="(item, index) in option" :key='index' :label='item.groupName' :value='item.groupId'></el-option>
                </el-select>
          ...
          <el-col :span="12">
            <el-form-item label="策略描述：">
              <el-input type="textarea" v-model="$store.state.strategy.strategyDesc"/>
            </el-form-item>
          </el-col>
          <el-col :span="12">
          ...
          <el-col :span="12">
            <el-form-item label="发送字段：">
              ...
              <el-button @click="tageNameVisibleShow">编辑</el-button>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="发送方式：">
              <el-select v-model="$store.state.strategy.sendType" :disabled='disabled'>
      ...
      <div class="headTitle">
        <div class="flex spaceBetween">
          <span class="bigSize"><img src="@/assets/indexImg/项目符.png" id="headLogo" alt="">策略动作</span>
          <div class="messageControl">
            <span class="cursor backSpan" @click="messageSet"><img src="../../assets/strategy/addlabel.png" alt="">统计指标编辑</span>
            <span class="cursor backSpan" @click="addAction"><img class="btnImg" src="../../assets/strategy/abc.png"/>添加规则</span>
          </div>
        </div>
        <div class="border"></div>
      </div>
      <Action v-for="(item, index) in $store.state.strategy.sitConditions" :key='index' :params='index' ref='action' />
      <AdminSend :props='{typeParams: false}' />
      <div class="headTitle">
        <div class="flex spaceBetween"><span class="bigSize"><img src="@/assets/indexImg/项目符.png" id="headLogo" alt="">策略关注指标</span><span class="cursor backSpan" @click="addStrategy"><img class="btnImg" src="../../assets/strategy/vbn.png"/>添加指标</span></div>
        <div class="border"></div>
      </div>
      <div class="sitStratrgyTegTags">
        <div class="sitStratrgyTegTag" v-for="(item, index) in $store.state.strategy.sitStratrgyTegTags" :key='index'>
          <el-form label-width="100px">
            ...
            <el-form-item label="指标分组">
              <span v-if="item.tagValue !== '2'" @click="getFenZu(index, item)" class="groupingSettings">分组设置</span>
              <span v-if="item.tagValue === '2'" class="groupingSettings">指标为枚举类型</span>
            </el-form-item>
          </el-form>
          <!-- 指标分组设置 -->
            <el-dialog :visible.sync="$store.state.strategy.sitStratrgyTegTags[index].factorDialogVisible" title='分组设置' class='dialog_footer fzClass' width="900px" :close-on-click-modal="false">
              <div class="logo_message"></div>
              <div class="footer_top">
                <p>指标： <span style='fontSize: 14px'>{{ item.name }}</span></p>
                <div class="addBtn">
                  <div @click="addGroup(index)" class="addBtn" style='cursor: pointer;'>
                  <img class="btnImg" style="padding-bottom: 2px;" src="../../assets/indexImg/添加.png"/>
                  <p style="fontSize: 14px; marginRight: 30px">添加组别</p>
                </div>
                  <p>分组模板：</p>
                  <el-select v-model="$store.state.strategy.sitStratrgyTegTags[index].optionIndex" @change="(e) => handleChangeTemplates(e, index)">
                    <el-option v-for="(item, index) in $store.state.strategy.sitStratrgyTegTags[index].option" :key='index' :value='item.templateName' :label='item.templateName' />
                  </el-select>
                  <span class="newTemplate" @click="handleAddTagGroupTemplate">另存为新模板</span>
                </div>
              </div>
              <el-form label-width="100px" class="fzClass">
                ...
                >
                  <div class="alignItem">
                    <el-input v-model="$store.state.strategy.sitStratrgyTegTags[index].sitStrategyTags[itemIndex].groupName" />
                    <div style="width: 40px;"></div>
                    <el-input v-model="$store.state.strategy.sitStratrgyTegTags[index].sitStrategyTags[itemIndex].startValue" />
                    <el-select v-model="$store.state.strategy.sitStratrgyTegTags[index].sitStrategyTags[itemIndex].leftOperator">
                      <el-option label="<" value="<"></el-option>
                      <el-option label="<=" value="<="></el-option>
                    </el-select>
                    <p style="fontSize: 14px;" > 值 </p>
                    <el-select v-model="$store.state.strategy.sitStratrgyTegTags[index].sitStrategyTags[itemIndex].rightOperator">
                      <el-option label="<" value="<"></el-option>
                      <el-option label="<=" value="<="></el-option>
                    </el-select>
                    <el-input v-model="$store.state.strategy.sitStratrgyTegTags[index].sitStrategyTags[itemIndex].endValue" />
                    <span
        ...
        <!-- 添加指标 -->
        <el-dialog :visible.sync="$store.state.strategy.strategyVisible" title='添加指标' width="317" class='add_dialog' :close-on-click-modal="false">
          ...
          <span slot="footer" class="dialog-footer">
            <el-button @click="$store.state.strategy.strategyVisible = false">取 消</el-button>
            <el-button type="primary" @click="addStrategyTags(index)">确 定</el-button>
          </span>
        </el-dialog>
        <el-dialog :visible.sync="tageNameVisible" title='发送字段' width="873px" class="send_dialog" :close-on-click-modal="false">
          ...
          <div class="el-dialog-div">
            ...
            <div class="rules_db">
              <p style='width: 55px; height: 35px; lineHeight: 35px; color: #000; textAlign: center;'>工号</p>
              <p style='width: 55px; height: 35px; lineHeight: 35px; color: #000; textAlign: center;'>姓名</p>
              <p style='width: 100px; height: 35px; lineHeight: 35px; color: #000; textAlign: center;'>分公司代码</p>
              <p style='width: 120px; height: 35px; lineHeight: 35px; color: #000; textAlign: center;'>三级机构代码</p>
              <p style='width: 120px; height: 35px; lineHeight: 35px; color: #000; textAlign: center;'>三级机构名称</p>
              <p style='width: 120px; height: 35px; lineHeight: 35px; color: #000; textAlign: center;'>归属机构代码</p>
              <p style='width: 120px; height: 35px; lineHeight: 35px; color: #000; textAlign: center;'>归属机构名称</p>
              <p style='width: 120px; height: 35px; lineHeight: 35px; color: #000; textAlign: center;'>从人渠道代码</p>
              <p style='width: 120px; height: 35px; lineHeight: 35px; color: #000; textAlign: center;'>从人渠道名称</p>
              <p style='width: 120px; height: 35px; lineHeight: 35px; color: #000; textAlign: center;'>所属团队代码</p>
              <p style='width: 120px; height: 35px; lineHeight: 35px; color: #000; textAlign: center;'>所属团队名称</p>
              <p style='width: 80px; height: 35px; lineHeight: 35px; color: #000; textAlign: center;'>岗位名称</p>
            </div>
            <div class="flex">
              <div class="textarea">
                <el-input type="textarea" readonly v-model="$store.state.strategy.tagNames" />
              </div>
          ...
          <span slot="footer" class="dialog-footer">
            <el-button type="primary" @click="tageNameVisible = false">确 定</el-button>
          </span>
        </el-dialog>
      </div>
      <div class="btnList">
        <div v-if="$store.state.strategy.strategyStatus === '1'">
          <el-button :loading='loading' @click="strategyUpdate(1)">提交审核</el-button>
          <el-button :loading='loading' @click="strategyUpdate(5)">取消</el-button>
          <el-button @click="goBack">返回</el-button>
        </div>
        <!-- 运行中 -->
        <div v-if="$store.state.strategy.strategyStatus === '4'">
          <el-button :loading='loading' @click="updateJobStatus(3)">暂停</el-button>
          <el-button :loading='loading' @click="updateJobStatus(6)">强制结束</el-button>
          <el-button @click="goBack">返回</el-button>
        </div>
        <!-- 未通过 -->
        <div v-if="$store.state.strategy.strategyStatus === '2'">
          <el-button @click="goBack">返回</el-button>
          <el-button :loading='loading' @click="strategyUpdate(1)">提交审核</el-button>
          <el-button :loading='loading' @click="strategyUpdate(5)">取消</el-button>
        </div>
        <!-- 已暂停 -->
        <div v-if="$store.state.strategy.strategyStatus === '3'">
          <el-button :loading='loading' @click="strategyUpdate(1)">提交审核</el-button>
          <el-button :loading='loading' @click="updateJobStatus(4)">恢复</el-button>
          <el-button :loading='loading' @click="updateJobStatus(6)">强制结束</el-button>
          <el-button @click="goBack">返回</el-button>
        </div>
        <!-- 强制结束 -->
        <div v-if="$store.state.strategy.strategyStatus === '6'">
          <el-button @click="goBack">返回</el-button>
        </div>
        <!-- 已取消 -->
        <div v-if="$store.state.strategy.strategyStatus === '5'">
          <el-button @click="goBack">返回</el-button>
        </div>
        <!-- 已完成 -->
        <div v-if="$store.state.strategy.strategyStatus === '7'">
          <el-button @click="goBack">返回</el-button>
        </div>
      </div>
    </div>
      ...
      <div class="messageDialog">
        ...
        <div class="messageDialogRight">
          <el-form style="width: 100%" class="messageDialogForm">
            ...
            <el-table border="true" class="messageDialogTable" :data="message.nowList.enumValueList" height="200">
              <el-table-column label="枚举值">
                <template slot-scope="res">{{res.row}}</template>
              </el-table-column>
              <el-table-column label="操作">
                <template slot-scope="res"><div class="msgDel" @click='deleteMessageList(res)'><img src="../../assets/strategy/delete.png" alt=""><p>删除</p></div></template>
              </el-table-column>
            </el-table>
          </el-form>
          <div>
...
import { getGroupTemplates, addTagGroupTemplate
// , getGroupId
} from '@/api/nameList'
...
export default {
  ...
  created () {
    ...
    if (this.$store.state.strategy.dockSystems === '3' || this.$store.state.strategy.dockSystems === '2') {
      this.disabled = false
    } else {
      this.disabled = true
    }
    this.getData()
    this.$store.state.strategy = { // 策略部分
      groupList: [], // 弹出框选项树
      switch: false,
      sitConditions: [], // 策略动作
  ...
  watch: {
    ...
    '$store.state.strategy.groupId' () {
      findAllTagClassAndTagCustom(this.$store.state.strategy.groupId).then((res) => {
  ...
  methods: {
    ...
    handleAddMessageList () { // 添加枚举值
      ...
      for (let i = 0; i < this.message.nowList.enumValueList.length; i++) {
        if (this.message.nowList.nameAdd === this.message.nowList.enumValueList[i]) {
    ...
    handleChangeTemplates (e, index) {
      for (let i = 0; i < this.$store.state.strategy.sitStratrgyTegTags[index].option.length; i++) {
        if (e === this.$store.state.strategy.sitStratrgyTegTags[index].option[i].templateName) {
          this.$store.state.strategy.sitStratrgyTegTags[index].sitStrategyTags = this.$store.state.strategy.sitStratrgyTegTags[index].option[i].groupSwarmsList
        }
      }
    },
    editTagName () {
      if (this.$store.state.strategy.groupId) {
        ...
        setTimeout(() => {
          this.$refs.treeMessage.setCheckedKeys(this.$store.state.strategy.tagIds)
        }, 0)
      } else {
    ...
    gotoGroupManagement () {
      if (this.$store.state.strategy.groupId) {
        setStrategyReturnDate(this.$store.state.strategy).then((res) => {
          this.$router.push('/groupManagement/see?history=/strategy/update&firId=' + res.data.data + '&groupId=' + this.$store.state.strategy.groupId)
        })
      } else {
    ...
    getFenZu (index, item) {
      ...
      this.$store.state.strategy.sitStratrgyTegTags[index].factorDialogVisible = true
    },
    groupClose (index) {
    ...
    strategyUpdate (e) {
      ...
      for (let i = 0; i < this.$store.state.strategy.sitConditions.length; i++) {
        for (let j = 0; j < this.$store.state.strategy.tagEnumList.length; j++) {
          for (let k = 0; k < this.$store.state.strategy.sitConditions[i].strategyTagEnumDtoList.length; k++) {
            if (j === k) {
              this.$store.state.strategy.sitConditions[i].strategyTagEnumDtoList[k].tagName = this.$store.state.strategy.tagEnumList[j].tagName
            }
          }
        }
      }
      ...
      if (this.$store.state.strategy.strategyStartTime >= this.$store.state.strategy.strategyEndTime) {
      ...
      if (this.$store.state.strategy.dockSystems === '2' || this.$store.state.strategy.dockSystems === '3') {
        console.log('判断')
      ...
      for (let i = 0; i < this.$store.state.strategy.sitConditions.length; i++) {
        for (let j = 0; j < this.$store.state.strategy.sitConditions[i].sitConditionFactors.length; j++) {
          if (this.$store.state.strategy.sitConditions[i].sitConditionFactors[j].tagValueType !== '2') {
            if (this.$store.state.strategy.sitConditions[i].sitConditionFactors[j].tagValue === undefined || this.$store.state.strategy.sitConditions[i].sitConditionFactors[j].tagValue === '') {
              this.$message.warning(`情况${i + 1}里${this.$store.state.strategy.sitConditions[i].sitConditionFactors[j].name}因子没写值`)
              this.error = true
              return
            }
            if (this.$store.state.strategy.sitConditions[i].sitConditionFactors[j].sitOperator === undefined || this.$store.state.strategy.sitConditions[i].sitConditionFactors[j].sitOperator === '') {
              if (this.$store.state.strategy.sitConditions[i].sitConditionFactors[j].tagValueType !== '2') {
                this.$message.warning(`情况${i + 1}里${this.$store.state.strategy.sitConditions[i].sitConditionFactors[j].name}因子没写运算符`)
                this.error = true
                return
              }
            }
            if (isNaN(this.$store.state.strategy.sitConditions[i].sitConditionFactors[j].tagValue) && this.$store.state.strategy.sitConditions[i].sitConditionFactors[j].tagValueType === '1') {
              this.$message.warning(`情况${i + 1}里${this.$store.state.strategy.sitConditions[i].sitConditionFactors[j].name}因子值不是数字`)
              this.error = true
              return
            }
          } else if (this.$store.state.strategy.sitConditions[i].sitConditionFactors[j].tagValueType === '2') {
            console.log(this.$store.state.strategy.sitConditions[i].sitConditionFactors[j].tagEnumValues)
            if (this.$store.state.strategy.sitConditions[i].sitConditionFactors[j].tagEnumValues.length === 0) {
              this.$message.warning(`情况${i + 1}里${this.$store.state.strategy.sitConditions[i].sitConditionFactors[j].name}因子没写值`)
              this.error = true
              return
            }
          } else {
            if (this.$store.state.strategy.sitConditions[i].sitConditionFactors[j].tagEnumValues === undefined || this.$store.state.strategy.sitConditions[i].sitConditionFactors[j].tagEnumValues === '') {
              this.$message.warning(`情况${i + 1}里${this.$store.state.strategy.sitConditions[i].sitConditionFactors[j].name}因子没写值`)
              this.error = true
              return
            }
      ...
      for (let i = 0; i < this.$store.state.strategy.sitStratrgyTegTags.length; i++) {
        if (this.$store.state.strategy.sitStratrgyTegTags[i].tagValue === '2') {
          for (let j = 0; j < this.$store.state.strategy.sitStratrgyTegTags[i].sitStrategyTags.length; j++) {
            if (this.$store.state.strategy.sitStratrgyTegTags[i].sitStrategyTags[j].groupName === '') {
              this.$message.warning(`指标里${i + 1}里分组${j + 1}名称为空`)
              this.error = true
              return
            }
            if (this.$store.state.strategy.sitStratrgyTegTags[i].sitStrategyTags[j].startValue === '' && this.$store.state.strategy.sitStratrgyTegTags[i].sitStrategyTags[j].endValue === '') {
              this.$message.warning(`指标里${i + 1}里分组${j + 1}至少填写一个值`)
              this.error = true
              return
            }
            if (isNaN(this.$store.state.strategy.sitStratrgyTegTags[i].sitStrategyTags[j].startValue)) {
              this.$message.warning(`指标里${i + 1}里分组${j + 1}开始值不是数字`)
              this.error = true
            }
            if (isNaN(this.$store.state.strategy.sitStratrgyTegTags[i].sitStrategyTags[j].endValue)) {
              this.$message.warning(`指标里${i + 1}里分组${j + 1}结束值不是数字`)
              this.error = true
            }
            if (this.$store.state.strategy.sitStratrgyTegTags[i].sitStrategyTags.length === 0) {
      ...
      for (let i = 0; i < params.sitStratrgyTegTags.length; i++) {
        if (params.sitStratrgyTegTags[i].tagValue !== '2' && params.sitStratrgyTegTags[i].sitStrategyTags.length === 0) {
        ...
        for (let i = 0; i < this.$store.state.strategy.sitStratrgyTegTags.length; i++) {
          ...
          if (this.$store.state.strategy.sitStratrgyTegTags[i].tagValue !== '2') {
            for (let j = 0; j < this.$store.state.strategy.sitStratrgyTegTags[i].sitStrategyTags.length; j++) {
              paramsList.push({
                ...
                startValue: this.$store.state.strategy.sitStratrgyTegTags[i].sitStrategyTags[j].startValue,
                endValue: this.$store.state.strategy.sitStratrgyTegTags[i].sitStrategyTags[j].endValue,
                groupName: this.$store.state.strategy.sitStratrgyTegTags[i].sitStrategyTags[j].groupName,
                leftOperator: this.$store.state.strategy.sitStratrgyTegTags[i].sitStrategyTags[j].leftOperator,
                rightOperator: this.$store.state.strategy.sitStratrgyTegTags[i].sitStrategyTags[j].rightOperator
              })
            }
          }
          checkAddGroup(paramsList).then((res) => {
            if (res.data.code && res.data.code !== 0) {
              ...
              if (i === this.$store.state.strategy.sitStratrgyTegTags.length - 1) {
                if (this.$store.state.strategy.strategyStartTime) {
                  this.$store.state.strategy.strategyStartTime = this.$store.state.strategy.strategyStartTime + ' 00:00:00'
                }
                if (this.$store.state.strategy.strategyEndTime) {
    ...
    updateJobStatus (e) {
      ...
      for (let i = 0; i < this.$store.state.strategy.sitConditions.length; i++) {
        if (this.$store.state.strategy.sitConditions[i].sitMessagev === '' && this.$store.state.strategy.sitConditions[i].strategyTagEnumDtoList.findIndex(findEnumvalue) === 0) {
    ...
    setSitStrategyTags (e) {
      for (let j = 0; j < this.$store.state.strategy.sitStratrgyTegTags[this.getFenZuNum].sitStrategyTags.length; j++) {
        if (this.$store.state.strategy.sitStratrgyTegTags[this.getFenZuNum].sitStrategyTags[j].groupName === '') {
          this.$message.warning(`指标里${this.getFenZuNum + 1}里分组${j + 1}名称为空`)
          this.error = true
          return
        }
        if (this.$store.state.strategy.sitStratrgyTegTags[this.getFenZuNum].sitStrategyTags[j].startValue === '' && this.$store.state.strategy.sitStratrgyTegTags[this.getFenZuNum].sitStrategyTags[j].endValue === '') {
          this.$message.warning(`指标里${this.getFenZuNum + 1}里分组${j + 1}至少填写一个值`)
          this.error = true
          return
        }
        if (isNaN(this.$store.state.strategy.sitStratrgyTegTags[this.getFenZuNum].sitStrategyTags[j].startValue)) {
          this.$message.warning(`指标里${this.getFenZuNum + 1}里分组${j + 1}开始值不是数字`)
          this.error = true
        }
        if (isNaN(this.$store.state.strategy.sitStratrgyTegTags[this.getFenZuNum].sitStrategyTags[j].endValue)) {
          this.$message.warning(`指标里${this.getFenZuNum + 1}里分组${j + 1}结束值不是数字`)
          this.error = true
        }
      }
      if (this.$store.state.strategy.sitStratrgyTegTags[this.getFenZuNum].sitStrategyTags.length === 0) {
      ...
      for (let j = 0; j < this.$store.state.strategy.sitStratrgyTegTags[this.getFenZuNum].sitStrategyTags.length; j++) {
        paramsList.push({
          startValue: this.$store.state.strategy.sitStratrgyTegTags[this.getFenZuNum].sitStrategyTags[j].startValue,
          endValue: this.$store.state.strategy.sitStratrgyTegTags[this.getFenZuNum].sitStrategyTags[j].endValue,
          groupName: this.$store.state.strategy.sitStratrgyTegTags[this.getFenZuNum].sitStrategyTags[j].groupName,
          leftOperator: this.$store.state.strategy.sitStratrgyTegTags[this.getFenZuNum].sitStrategyTags[j].leftOperator,
          rightOperator: this.$store.state.strategy.sitStratrgyTegTags[this.getFenZuNum].sitStrategyTags[j].rightOperator
        })
      }
      checkAddGroup(paramsList).then((res) => {
        if (res.data.code !== 0) {
    ...
    handleAddTagGroupTemplate () {
      console.log(this.$store.state.strategy.sitStratrgyTegTags[this.getFenZuNum])
      for (let j = 0; j < this.$store.state.strategy.sitStratrgyTegTags[this.getFenZuNum].sitStrategyTags.length; j++) {
        if (this.$store.state.strategy.sitStratrgyTegTags[this.getFenZuNum].sitStrategyTags[j].groupName === '') {
          this.$message.warning(`指标里${this.getFenZuNum + 1}里分组${j + 1}名称为空`)
          this.error = true
    ...
    addOne () {
      ...
      const params = {
        ...
        tagId: this.$store.state.strategy.sitStratrgyTegTags[this.getFenZuNum].id,
        tagName: this.$store.state.strategy.sitStratrgyTegTags[this.getFenZuNum].name,
      ...
      addTagGroupTemplate(params).then((ress) => {
        if (ress.data.code === 0) {
          ...
          this.getTemplateNameVisible = false
          this.$store.state.strategy.sitStratrgyTegTags[this.getFenZuNum].optionIndex = this.templateName
          this.templateName = ''
          console.log('456')
          getGroupTemplates({ tagId: this.$store.state.strategy.sitStratrgyTegTags[this.getFenZuNum].id }).then((res) => {
...
/deep/ .add_dialog .el-dialog .el-dialog__header .el-dialog__headerbtn .el-dialog__close{
  font-size: 28px;
}
...
/deep/ .send_dialog .el-dialog .el-dialog__header .el-dialog__headerbtn .el-dialog__close{
  font-size: 28px;
}
...
/deep/ .send_dialog .el-dialog .el-dialog__body .textarea .el-textarea .el-textarea__inner{
